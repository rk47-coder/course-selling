...
//   // fetch courses
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await axios.get("http://localhost:4001/api/vi/course/courses",{ withCredentials: true,  });
  ...
  const handleLogout = async () => {
    try {
      const response = await axios.get(
        ...
        "http://localhost:4001/api/vi/user/logout", {
                ...
                <div className="relative flex-shrink-0 w-80 transition-transform duration-300 transform hover:scale-105">
                  <div className="bg-gray-900 rounded-lg overflow-hidden">
                    <img
                      ...
                      src={course.image?.url || "default_image_url_here"} // Use a default image URL if course.image is missing
                      alt={course.title || "Course image"}
                    />
                    <div className="p-6 text-center">
                      <h2 className="text-xl mt-0 mb-6 font-bold text-white">
                        {course.title}
                      </h2>
